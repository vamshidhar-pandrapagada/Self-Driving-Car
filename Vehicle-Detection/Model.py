...
def find_cars(img, scale, y_start_stop):
...
def extract_train_features(annotated_features = False):
    ...
    print('Number of Car Images: ' +str(len(car_images)))
    print('Number of Non Car Images: ' +str(len(not_car_images)))
...
def model_train(X, y):    
    ...
    if model_name =='SVM':
        ...
        """
        pipe_svc = Pipeline([('clf', LinearSVC(random_state = 1))])
        ...
        print('Accuracy of SVC on Train Set = ', round(gs.best_score_, 4))    
        print('Best Grid Parameters = ', gs.best_params_)
        
        best_clf = gs.best_estimator_    
        # Save Best Classifier to Pickle
        with open(save_model_path + 'Best SVC_Classifier' + '.pkl', 'wb') as output:  
                best_clf.fit(X_train, y_train)
                print('Test Accuracy of SVC = ', round(best_clf.score(X_test, y_test), 4))
                pickle.dump(best_clf, output, pickle.HIGHEST_PROTOCOL) 
...
def extract_test_features(img, windows):
    features = [] 
    spatial_features = []
    color_hist_features = []
    hog_features = []
    for window in windows:
        ...
        img_converted = convert_color(test_img, cspace = color_space)
        if spatial_feat:
            spatial_features = bin_spatial(img_converted, spatial_size)
        ...
        #window_and_features[window] = np.concatenate([spatial_features, color_hist_features, hog_features])
        features.append(np.concatenate([spatial_features, color_hist_features, hog_features]))      
...
def  model_predict(test_image, model_name, X_scaler, scales, y_start_stop_list, 
    ...
    for scale, y_start_stop in zip(scales, y_start_stop_list):    
        all_features, boxes = find_cars(test_image, scale, y_start_stop)
...
def model_test(test_image):
    ...
    #print('Test Features Shapes:', all_test_features.shape)       
    with open(save_model_path + 'scale_features_X.pkl', 'rb') as inputs:
            X_scaler = pickle.load(inputs)          
    ...
    all_boxes = []
    car_detected_boxes = []
    for xy_window, y_start_stop in zip(xy_window_scales, y_start_stop_list):
        windows = slide_window(test_image, x_start_stop = x_start_stop, y_start_stop = y_start_stop, 
                                xy_window = xy_window, xy_overlap = xy_overlap)
        
        all_boxes.append(windows)
    all_boxes = list(chain(*all_boxes))        
    test_features = extract_test_features(test_image , all_boxes)
    
    predictions = model_predict(test_features, model_name)   
    print(len(predictions))
    print(len([i for i in predictions if  i == 1]))    
    for i, pred in enumerate(predictions):
        if pred == 1:
            #cv2.rectangle(draw_image , all_boxes[i][0], all_boxes[i][1], (255,0,0), 6) 
            car_detected_boxes.append(all_boxes[i])
